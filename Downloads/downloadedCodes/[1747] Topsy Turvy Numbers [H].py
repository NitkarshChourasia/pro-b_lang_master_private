"""
####  Topsy Turvy Numbers  ####

Topsy turvy numbers are numbers that when inverted (rotated 180 degrees) are unchanged. The most recent topsy turvy years were 1961 and 1881. The next one won't be until 6009. Mathemeticians have shown little interest in these numbers, but Edabitians are always up for a challenge:
Write a function that accepts two non-negative integers and returns a list of all topsy turvy numbers inclusively in that range.


[Examples]

___
topsy_turvy(0, 10) ➞ [0, 1, 8]

topsy_turvy(10, 100) ➞ [11, 69, 88, 96]

topsy_turvy(1000, 2000) ➞ [1001, 1111, 1691, 1881, 1961]
_____



[Notes]

N/A


[loops] [numbers] [strings] 



-------------------------------------------------------------------
[Resources]
_________
Strobogrammatic Numbers
http://www.numbersaplenty.com/set/strobogrammatic_number/
A number is called strobogrammatic if it remains the same when read upside down. Usually strobogrammatic numbers are made only of digits 0, 1, 6, 8, and 9. However, sin …
_________

"""
#Your code should go here:

