"""
####  Last Digits of a Huge Fibonacci Number  ####

Create a function that takes a number n and returns the last four digits of nth Fibonacci number.
In this challenge, the given number n may be huge (billions or so). Hence an algorithm looping for n iterations will not fit into the allotted time. Therefore you have to find out the algorithm which finds the answer in O(log n) time.


[Examples]

___
fibonacci(6) ➞ 8

fibonacci(10) ➞ 55

fibonacci(10000000) ➞ 6875

fibonacci(12345678901) ➞ 7401
_____



[Notes]

The Fibonacci numbers are defined as follows:
___
f(0) = 0, f(1) = 1, and f(i) = f(i−1) + f(i−2)
_____



[algebra] [algorithms] [logic] [loops] [numbers] 



-------------------------------------------------------------------
[Resources]
_________
Last 10 digits of the billionth fibonacci number?
https://math.stackexchange.com/questions/1353119/last-10-digits-of-the-billionth-fibonacci-number
I want to compute the last ten digits of the billionth fibonacci number, but my notebook doesn't even have the power to calculate such big numbers, so I though of a ver …
_________
_________
Fibonacci Number
https://en.wikipedia.org/wiki/Fibonacci_number
Form a sequence, called the Fibonacci sequence, such that each number is the sum of the two preceding ones, starting from 0 and 1.
_________
_________
Pisano period
https://en.wikipedia.org/wiki/Pisano_period
Are named after Leonardo Pisano, better known as Fibonacci.
_________

"""
#Your code should go here:

